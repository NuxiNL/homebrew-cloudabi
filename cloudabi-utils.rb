class CloudabiUtils < Formula
  ...
  def install
    system "cmake", ".", "-DCMAKE_C_COMPILER=#{Formula["llvm@4"].opt_bin}/clang", "-DCMAKE_CXX_COMPILER=#{Formula["llvm@4"].opt_bin}/clang++", "-DCMAKE_C_FLAGS=\"-I#{Formula["cloudabi"].include}\"", *std_cmake_args
    system "make"
    system "make", "install"
  end
end
